...
### 2026-05-04: Ổn định môi trường Build & Loại bỏ Excel
- **Trạng thái**: Hoàn thành (Build OK - 0 Errors).
- **Các thay đổi chính**:
    - Loại bỏ hoàn toàn phụ thuộc vào `Microsoft.Office.Interop.Excel` và `Microsoft.Office`. Sử dụng các giải pháp thay thế (nếu cần) hoặc loại bỏ logic không sử dụng.
    - Sửa lỗi build `InitializeComponent` bằng cách sử dụng `MSBuild.exe` thay vì `dotnet build` cho project .NET Framework 4.8 legacy.
    - Bổ sung các tham chiếu NuGet thiếu (`Logging`, `Primitives`) vào file `.csproj` để đồng bộ với `packages.config`.
    - Thêm hỗ trợ build cho cấu hình `Debug|AnyCPU` để đảm bảo tính linh hoạt.
- **Lưu ý build**: Luôn sử dụng `MSBuild.exe` từ Visual Studio để build project này để đảm bảo các file XAML được biên dịch đúng (`MarkupCompilePass`).

---

## 🛠 Danh sách công việc (Task List)
- [x] **Giai đoạn 0**: Ổn định môi trường build & Dependency Cleanup.
- [ ] **Giai đoạn 1**: Cấu hình DI & MVVM Foundation.
- [/] **Giai đoạn 2**: Bóc tách logic PLC & Database Services. (Đang thực hiện)
